...
export default function Sidebar({ activeTab, onTabChange, isSuperuser = false }: SidebarProps) {
  ...
  return (
    <aside className="w-72 glass border-r border-white/50 flex flex-col sticky top-0 h-screen">
      ...
      <nav className="flex-1 p-4 space-y-2">
        ...
        >
          <div className={`w-9 h-9 rounded-xl flex items-center justify-center ${
            activeTab === 'novel' ? 'bg-white/20' : 'bg-slate-100'
          }`}>
            <svg className={`w-5 h-5 ${activeTab === 'novel' ? 'text-white' : 'text-slate-500'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
            </svg>
          </div>
          <div className="flex-1">
            <div className="font-medium">小说转剧本</div>
            <div className={`text-xs ${activeTab === 'novel' ? 'text-white/70' : 'text-slate-400'}`}>AI 一键转分镜剧本</div>
          </div>
          <span className={`text-xs px-2 py-0.5 rounded-full ${
            activeTab === 'novel' ? 'bg-white/20 text-white' : 'bg-slate-200 text-slate-500'
          }`}>2</span>
        </button>
        ...
        {isSuperuser && (
          ...
          >
            <div className={`w-9 h-9 rounded-xl flex items-center justify-center ${
              activeTab === 'admin' ? 'bg-white/20' : 'bg-slate-100'
            }`}>
              <svg className={`w-5 h-5 ${activeTab === 'admin' ? 'text-white' : 'text-slate-500'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
              </svg>
            </div>
            <div className="flex-1">
              <div className="font-medium">管理员</div>
              <div className={`text-xs ${activeTab === 'admin' ? 'text-white/70' : 'text-slate-400'}`}>账号与激活管理</div>
            </div>
          </button>
        )}
