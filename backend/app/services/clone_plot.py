...
async def get_ori_context(state: ClonePlotState) -> Command[Literal['process_error', 'creative_plot']]:
...
async def creative_plot(state: ClonePlotState) -> Command[Literal['process_error', '__end__']]:
    ...
    async with process_loop.AsyncSessionLocal() as db:
        try:
            ...
            logger.info(f'analysis_focus is {state['ori_parameters'].get('analysis_focus')}')
            logger.info(f'plot_script is {state['ori_parameters'].get('plot_script')}')
            logger.info(f'clone_theme is {clone_script.clone_theme}')

            focus_output = await creative_focus_output(
            ...
            if not isinstance(focus_output, CloneAnalysisFocus):
                raise Exception("llm输出格式错误")
            
            plot_output = await creative_plot_output(
                ori_plot_script=state['ori_parameters'].get('plot_script'),
                new_plot_focus=focus_output.model_dump()
            )
            if not isinstance(plot_output, CloneAnalysisPlot):
                raise Exception("llm输出格式错误")
            
            clone_analysis = CloneAnalysis(
                **focus_output.model_dump(),
                **plot_output.model_dump()
            )
            
            logger.info(f'generate CloneAnalysis: {clone_analysis.model_dump_json()}')
            for plot in clone_analysis.plot_script:
            ...
            # 估算时长，修改对话文本
            for item_script in clone_analysis.plot_script:
                ...
                if predict_duration > item_script_duration*1.05:
            ...
            logger.info(f'convert_clone_to_md response is {clone_analysis}')
            md_path = await convert_clone_to_md(clone_analysis, dir_path, state['clone_script_id'])
...
clone_plot_graph_builder.add_node("creative_plot", creative_plot)
