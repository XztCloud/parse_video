...
from app.models.script import CloneVoice, CloneScript, CloneScriptSegment, CloneVideo, Script, CloneStatus
...
@router.post("/plot")
async def clone_plot(request: ClonePlotRequest):
    try:
...
async def get_clone_status(clone_script_id: int):
    try:
        print(f'receive get_clone_status. sciript_id is {clone_script_id}')
        db = SessionLocal()
        try:
...
async def list_clone_scripts(script_id: int, offset:int=0, limit:int=20):
    try:
...
async def get_clone_scirpt(clone_script_id: int):
    try:
        ...
        try:
            ...
            if clone_script.clone_status in plot_complete_status or \
                ...
                if clone_parse_file.is_file():
                    ...
                    voices = db.query(CloneVoice).filter(CloneVoice.script_id == clone_script.id).order_by(CloneVoice.created_at).all()
...
async def export_clone_script(clone_script_id: int):
    try:
        db = SessionLocal()
        try:
            clone_script = db.query(CloneScript).filter(CloneScript.id == clone_script_id).first()
...
@router.post("/voices")
async def clone_voices(request: CloneStoryboardRequest):
    db = SessionLocal()
    try:
...
async def clone_segments(request: CloneStoryboardRequest):
    try:
        db = SessionLocal()
        try:
            clone_script = db.query(CloneScript).filter(CloneScript.id == request.clone_script_id).first()
